...
class ArrayTest < Minitest::Test
  def skip_test_empty_keep
    assert_equal [], [].keep { |e| e < 10 }
  end

  def skip_test_keep_everything
    assert_equal [1, 2, 3], [1, 2, 3].keep { |e| e < 10 }
  end

  def skip_test_keep_first_and_last
    assert_equal [1, 3], [1, 2, 3].keep(&:odd?)
  end

  def skip_test_keep_neither_first_nor_last
    assert_equal [2, 4], [1, 2, 3, 4, 5].keep(&:even?)
  end

  def skip_test_keep_strings
  ...
  def skip_test_empty_discard
    assert_equal [], [].discard { |e| e < 10 }
  end

  def skip_test_discard_nothing
    assert_equal [1, 2, 3], [1, 2, 3].discard { |e| e > 10 }
  end

  def skip_test_discard_first_and_last
    assert_equal [2], [1, 2, 3].discard(&:odd?)
  end

  def skip_test_discard_neither_first_nor_last
    assert_equal [1, 3, 5], [1, 2, 3, 4, 5].discard(&:even?)
  end

  def skip_test_discard_strings
    words = %w(apple zebra banana zombies cherimoya zelot)
